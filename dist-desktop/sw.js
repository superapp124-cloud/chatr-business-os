...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    fetch(request)
      .then(response => {
        // Return the response even if it's 400, 401, 500 etc. so the frontend can handle it!
        return response;
      })
      .catch(async () => {
        // Only hits here on ACTUAL network failure (offline/cors)
        const cachedResponse = await caches.match(request);
        if (cachedResponse) {
          return cachedResponse;
        }
        // If it's an API request and we have no cache, we must return a generic 503 response
        // so we don't throw a TypeError!
