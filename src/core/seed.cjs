...
// src/core/memory/KnowledgeGraph.ts
  ...
  async addEdge(edge) {
    ...
    try {
      await db.execute(
        ...
        [edge.sourceId, edge.targetId, edge.relationship, edge.weight || 1, JSON.stringify(edge.attributes || {})]
      );
    } catch (e) {
      console.error("[KnowledgeGraph] Failed to add edge", e);
    }
  }
  async traverse(startNodeId, depth = 1) {
    const db = storageEngine.getAdapter();
...
async function seed() {
  ...
  await knowledgeGraph.addNode({
    ...
    attributes: { date: (/* @__PURE__ */ new Date()).toISOString() }
  });
