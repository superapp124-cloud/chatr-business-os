...
export class LogisticsWorkflow implements WorkflowCapabilityContract {
  readonly manifest: WorkflowManifest = {
    id: 'LOGISTICS_WORKFLOW',
    version: '1.0',
    name: 'Logistics',
    description: 'Courier, Parcel, Tracking',
  ...
  async initialize(context: WorkflowContext): Promise<void> {
    this.workflowId = buildWorkflowId(context.conversationId, 'logistics');
  }

  async execute(context: WorkflowContext): Promise<void> {
    await BrowserAutomationProvider.execute(this.workflowId, context, {
      targetDomain: 'Logistics Provider',
